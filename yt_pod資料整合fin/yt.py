...
api_key = "Your_Google_API" # Google API
...
def generate_summary(video_transcript):
    ...
    for part in transcript_parts:
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "你是一個針對財金相關內容的專業中文訊息整合生成器。"},
    ...
    final_response = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[
                {"role": "system", "content": "你是一個專業的中文訊息整合生成器。"},
...
for channel_url in channel_list:
    ...
    folder_path = f"{channel_name}的folder" #用名稱定義資料夾名稱
    ...
    for video in videos:
        ...
        transcription = transcribe_audio_with_whisper('Your_Path_of_mp3file')
